import { Link } from "react-router-dom";
import { useState, useEffect } from "react";
import { getGames, updateGame, deleteGame } from "../services/GameService";
import { FaTrashAlt } from "react-icons/fa";

const Home = () => {
  const [games, setGames] = useState([]);

  useEffect(() => {
    const getAllGames = async () => {
      const response = await getGames();
      const json = await response.json();

      if (!response.ok) {
        console.log(json.error);
      }

      if (response.ok) {
        console.log(json);
        setGames(json);
      }
    };

    getAllGames();
  }, []);

  const removeGame = async (id) => {
    const response = await deleteGame(id);

    const json = await response.json();

    if (response.ok) {
      setGames(games.filter((game) => json.gameId !== game.gameId));
    }
  };

  const updateGameStatus = async (e, id) => {
    const updatedStatus = {
      status: e.target.value,
    };

    const response = await updateGame(id, updatedStatus);

    const json = await response.json();

    if (response.ok) {
      setGames(
        games.map((game) => {
          if (json.gameId == game.gameId) {
            return json;
          }

          return game;
        }),
      );
    }
  };

  return (
    <div>
      <section className="mx-auto max-w-6xl flex-col items-start justify-between gap-4 p-4 text-slate-100 md:flex-row md:items-center md:gap-0">
        <h1 className="text-center text-4xl font-bold">
          Viewing the Games List
        </h1>
        {games.length > 0 && (
          <ul className="my-5 flex flex-col rounded-md p-2">
            {games.map((game) => (
              <li
                key={game.gameId}
                className="flex flex-col items-center border-b-2 border-slate-100 p-3 py-8 last:border-b-0 md:flex-row md:items-start"
              >
                <Link to={`/games/${game.gameId}`} className="hover:opacity-90">
                  {game.cover ? (
                    <img
                      src={game.cover}
                      alt={game.title}
                      className="mb-4 h-48 border border-slate-100 object-cover object-center md:mb-0 md:h-60"
                    />
                  ) : (
                    <div className="mb-4 flex h-48 w-44 items-center border border-slate-100 object-cover object-center text-center md:mb-0 md:h-60">
                      {game.title}
                    </div>
                  )}
                </Link>
                <div className="flex-1 px-6">
                  <div className="flex justify-between">
                    <Link
                      to={`/games/${game.gameId}`}
                      className="hover:opacity-90"
                    >
                      <h2 className="mb-4 text-2xl font-bold hover:opacity-90">
                        {game.title}
                      </h2>
                    </Link>
                    <FaTrashAlt
                      className="cursor-pointer self-start text-2xl text-white md:text-3xl"
                      onClick={() => removeGame(game.gameId)}
                    />
                  </div>
                  <p>{game.description}</p>
                  <div className="mt-4 flex flex-col space-y-4 font-bold text-white md:flex-row md:space-y-0 md:space-x-2">
                    <button
                      className={`cursor-pointer rounded-md px-6 py-3 hover:opacity-90 ${
                        game.status === "playing"
                          ? "bg-green-600"
                          : "bg-blue-900"
                      }`}
                      value="playing"
                      disabled={game.status === "playing"}
                      onClick={(e) => updateGameStatus(e, game.gameId)}
                    >
                      Playing
                    </button>
                    <button
                      className={`cursor-pointer rounded-md px-6 py-3 hover:opacity-90 ${
                        game.status === "completed"
                          ? "bg-green-600"
                          : "bg-blue-900"
                      }`}
                      value="completed"
                      disabled={game.status === "completed"}
                      onClick={(e) => updateGameStatus(e, game.gameId)}
                    >
                      Completed
                    </button>
                    <button
                      className={`cursor-pointer rounded-md px-6 py-3 hover:opacity-90 ${
                        game.status === "planning"
                          ? "bg-green-600"
                          : "bg-blue-900"
                      }`}
                      value="planning"
                      disabled={game.status === "planning"}
                      onClick={(e) => updateGameStatus(e, game.gameId)}
                    >
                      Planning
                    </button>
                  </div>
                </div>
              </li>
            ))}
          </ul>
        )}
      </section>
    </div>
  );
};

export default Home;
